...
pub fn load_node(
    ...
    for child in node.children() {
        load_node(ctx, &child, model, buffers, node_transform, path);
    }
    ...
    if let Some(mesh) = node.mesh() {
        ...
        for primitive in primitives {
            ...
            let mut vertices: Vec<PBRVertex> = vec![];

            for (i, _) in positions.iter().enumerate() {
                vertices.push(PBRVertex {
                    pos: positions[i].extend(0.0).into(),
            ...
            model.meshes.push(Mesh {
                primitive: Primitive::new(ctx, indices, vertices),
                material: Material {
...
pub fn load_gltf(ctx: &RenderContext, path: &str) -> Model {
    ...
    for image in &mut images {
        ...
        if image.format != gltf::image::Format::R8G8B8A8 {
            panic!("Unsupported image format! {:?}", image.format);
        }

        let raw_image = RawImage {
            extent: Extent3D { width: image.width, height: image.height, depth: 1 },
            format: ash::vk::Format::R8G8B8A8_UNORM,
            raw_bytes: image.pixels.clone(),
        };

        let texture = ctx.create_texture(&raw_image).unwrap();
        model.textures.push(texture);
    }

    for scene in gltf.scenes() {
        for node in scene.nodes() {
            load_node(ctx, &node, &mut model, &buffers, Mat4::IDENTITY, path);
        }
    }
