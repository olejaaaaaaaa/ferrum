...
impl WindowManager {
    ...
    pub fn resize(&mut self, dev: &Arc<GraphicsDevice>, width: u32, height: u32) {
        ...
        if caps.current_extent.width < width || caps.current_extent.height < height {
            return;
        }
        ...
        let swapchain = SwapchainBuilder::new()
            ...
            .with_extent(Extent2D { width, height })
            .with_format(format.format)
            .with_color_space(format.color_space)
            .with_present_mode(*mode)
            .with_min_count_image(3)
            .build()
            .unwrap();

        let swapchain_images = swapchain.get_swapchain_images().unwrap();
        ...
        let frame_buffers = FrameBuffersBuilder::default()
            .with_device(device)
            .with_render_pass(&self.render_pass.raw)
            .with_extent(Extent2D { width, height })
            .with_image_views(&image_views.raw)
            .build()
            .unwrap();
        ...
        unsafe {
            ...
            self.swapchain.swapchain_loader.destroy_swapchain(self.swapchain.raw, None);
        }
